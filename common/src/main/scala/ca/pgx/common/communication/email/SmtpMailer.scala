package ca.pgx.common.communication.email
...
/**
 * A Mailer config object that uses Props and auto configures for gmail if detected.
 */
object SmtpMailer extends Loggable {
  // FIXME: Loggable - which logger to use?
  def init(): Unit = {
